...
public final class AlertHistoryDao_Impl implements AlertHistoryDao {
    ...
    @Override
    public AlertHistoryEntity getFirstHistory(String str, int i) {
        return null;
    }
    ...
    public List<AlertHistoryEntity> getHistoryFilter(String str, long j, long j2) {
        ...
        String str4;
        String str5 = str;
        RoomSQLiteQuery acquire = RoomSQLiteQuery.acquire("SELECT * FROM AlertHistory WHERE user_name LIKE '%' || ? || '%' AND time >= ? AND time <= ? AND is_tmp = 0", 3);
        if (str5 == null) {
            acquire.bindNull(1);
        } else {
